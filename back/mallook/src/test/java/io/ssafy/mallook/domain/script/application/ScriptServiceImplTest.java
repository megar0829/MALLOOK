...
class ScriptServiceImplTest {
    ...
    void getScriptList() {
        ...
        Mockito.when(scriptRepository.findAllByMemberAndStatusTrue(proxyMember, pageable)).thenReturn(emptyPage);
        Page<ScriptListDto> result = scriptService.getScriptList(memberId, pageable);

        // then
        Mockito.verify(scriptRepository, Mockito.times(1)).findAllByMemberAndStatusTrue(proxyMember, pageable);
    ...
    void getScriptDetail() {
        ...
        // when
        Mockito.when(scriptRepository.findByIdAndStatusTrue(scriptId)).thenReturn(Optional.of(script));
        ScriptDetailDto result = scriptService.getScriptDetail(scriptId);

        Mockito.verify(scriptRepository, Mockito.times(1)).findByIdAndStatusTrue(scriptId);
