...
public class S3Util {
    ...
    // 파일 업로드
    @Transactional
    public File upload(String path, MultipartFile file) throws IOException {
        ...
        var s3File = File.builder()
                ...
                .s3DataUrl(fileUrl)
                .build();
        return fileRepository.save(s3File);
    }

    @Transactional
    public void remove(UUID id) {
        var s3File = fileRepository.findById(id)
    ...
    public byte[] getObject(String s3FileUrl) throws IOException {
        // bucket 와 dir 을 사용해서 S3 에 있는 객체 - object - 를 가져온다.
