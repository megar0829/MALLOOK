package io.ssafy.mallook.domain.file.controller;

import io.ssafy.mallook.domain.file.dto.response.FileRes;
import io.ssafy.mallook.domain.file.service.FileService;
...
public class FileController {
    ...
    @PostMapping("/{path}")
    public FileRes uploadFile(@PathVariable("path") String path,
    ...
    // name 과 파라미터로 넘어온 dir 을 getObject 메서드에 매개변수로 넣는다.
    @GetMapping("/{fileId}")
    public ResponseEntity<byte[]> download(@PathVariable(name = "fileId") UUID fileId) throws IOException {
        ...
        var byteFile = fileService.getObject(fileId);

        // 여기는 httpHeader 에 파일 다운로드 요청을 하기 위한내용
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.setContentType(MediaType.APPLICATION_OCTET_STREAM);

        // 지정된 name 으로 파일이 다운로드 된다.
